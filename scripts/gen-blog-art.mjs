...
const ART = {
  /* Ферментация для кожи головы: культура делится, из неё растут волокна.
     Абстракция процесса, а не изображение продукта. */
  'brewers-yeast-for-scalp-what-to-ask': shell(`
    <g opacity="0.8">${hexMesh(600, 250, 15, 9, 4)}</g>
    ${drop(360, 286, 1.2, 0.85)}
    ${drop(600, 250, 1.7, 1)}
    ${drop(840, 286, 1.2, 0.85)}
    ${strands(600, 300)}
  `, '50% 16%'),

  /* Сертификация: два документа разного веса — поимённый и общий.
     Печать стоит только на одном, в этом вся мысль статьи. */
  'vegan-certification-which-body-matters': shell(`
    ${sheet(330, 92, 200, 280, -6, true)}
    ${sheet(670, 104, 200, 280, 6, false)}
    ${arrow(545, 244, 655)}
    <rect x="330" y="398" width="540" height="6" rx="3" fill="url(#sheen)"/>
  `),

  /* Липосомальная доставка: заряженный носитель проходит слои рогового слоя.
     Три капли разного заряда, из них проходит одна. */
  'liposome-delivery-in-cosmetics': shell(`
    ${drop(360, 196, 1.15, 0.45)}
    ${drop(600, 168, 1.75, 1)}
    ${drop(840, 196, 1.15, 0.45)}
    ${layers(330, 322, 540)}
  `, '50% 14%'),
