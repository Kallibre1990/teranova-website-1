...
export const TEXTS = {
  ...
  ru: (s) => ({
    eyebrow: 'Teranova Group · профиль компании',
    tagline: 'Корейские производители — покупателям за рубежом',
    descriptor: 'B2B-платформа и сопровождение сделки целиком · Корея',

    who_h: 'Кто мы',
    who: [
      'Teranova Group — торговая платформа, зарегистрированная в Корее. Юридическое лицо — AIA Group Ltd., основатель — Антон Мадельканов. Мы работаем на стороне покупателя: партнёр приносит нам требование, мы находим производителя, который действительно ему отвечает, и дальше ведём сделку целиком — переговоры, документы, логистику, таможенное оформление и перевод.',
      'Мы не держим склад и ничего не перепродаём. Наше вознаграждение — комиссия с состоявшейся сделки, поэтому нам незачем продвигать одного производителя в ущерб другому. По той же причине мы прямо говорим, когда требование не совпадает с тем, что производитель на самом деле выпускает.',
      `Сегодня на платформе размещено ${s.total} производителей, из них ${s.korea} корейских. У каждого есть профиль компании и страницы продукции на одиннадцати языках. Мы собираем их из материалов, которые прислал сам производитель, и публикуем только после письменного согласия.`,
    ],
    who_facts: ['Зарегистрированы в Корее', 'Без склада и перепродажи', 'Оплата с состоявшейся сделки', 'Одиннадцать языков'],

    listing_h: 'Что даёт размещение',
    listing_lead: 'Публикация бесплатна для производителя и не создаёт для вас никаких обязательств.',
    listing: [
      ...
      { n: 'Документы из ваших же данных', d: 'Условия сотрудничества и презентация собираются из тех же данных, что и страница, поэтому покупатель может скачать их и показать внутри своей компании.' },
      { n: 'Не карточка, а рассказ', d: 'Мы пишем о технологии, которая стоит за вашей продукцией, чтобы покупатель, впервые услышавший о бренде, понял, чем он отличается.' },
      { n: 'Ваши контакты закрыты', d: 'Прямые контакты производителя мы не публикуем. Запросы приходят сначала к нам, мы их проверяем и только потом передаём вам.' },
    ],
    listing_note: 'Цены публикуются только по просьбе производителя. По умолчанию на страницах стоит «цена по запросу».',

    verify_h: 'Как мы представляем производителя',
    verify_lead: 'Покупатель должен понимать, откуда взят каждый факт. У нас ровно два статуса, и границу между ними мы не размываем.',
    verify: [
      { n: 'Проверено — личный визит', d: 'Основатель был в офисе или на производстве, встречался с руководством и делал фотографии. На странице стоят дата и снимки. Встреча на выставке визитом не считается.' },
      { n: 'Информация предоставлена компанией', d: 'Статус по умолчанию. Всё на странице взято из материалов производителя, и страница говорит об этом прямо.' },
    ],
    verify_note: 'Визит — это задокументированный факт о том, что мы видели сами. Сертификаты и регистрации показываются так, как их заявила компания; копии передаются покупателю на стадии сделки.',

    deal_h: 'Как идёт сделка',
    deal: [
      { n: 'Запрос', d: 'Покупатель приходит к нам. Мы проверяем страну, канал продаж, объём и то, способен ли он ввезти товар самостоятельно.' },
      { n: 'Конкретное задание', d: 'Мы приносим вам запрос с цифрами и условиями, а не просто имя компании. Если запрос не выдерживает проверки, вы о нём даже не узнаете.' },
      { n: 'Знакомство', d: 'Когда всё становится предметным, вы и покупатель садитесь за один стол в нашем присутствии — созвон или встреча. Информацию мы при этом не придерживаем.' },
      { n: 'Трёхсторонний договор', d: 'Его подписывают покупатель, производитель и Teranova. Оплата за товар идёт вам напрямую. Наша комиссия — отдельная строка в том же договоре, который вы подписываете и, значит, видите.' },
      { n: 'Исполнение', d: 'Логистика, документы, таможенное оформление и перевод: в договоре записано, что именно из этого мы берём на себя по конкретной сделке.' },
    ],

    cost_h: 'Во что это обходится вам',
    cost_lead: 'Размещение бесплатно, и до состоявшейся сделки вы не платите ничего.',
    cost: [
      { n: 'Публикация и обновления', d: 'Бесплатно. Новая продукция, сертификаты и новости компании добавляются в ваш профиль по мере того, как вы их присылаете.' },
      { n: 'Наша комиссия', d: 'Выплачивается с состоявшейся сделки и записывается в трёхсторонний договор. Это не наценка к вашей цене и не скрытый сбор. Ставка зависит от сделки и согласовывается до того, как что-либо подписано.' },
      { n: 'Что мы берём на себя', d: 'Указывается в договоре по каждой сделке отдельно, потому что объём работ меняется от сделки к сделке, и мы предпочитаем это записать, а не подразумевать.' },
    ],
    cost_note: 'Логистика и брокерские услуги считаются отдельно и показываются в счёте полностью.',

    markets_h: 'Рынки, каналы и категории',
    markets_sub1: 'Рынки',
    markets: ['Мексика', 'Бразилия', 'Аргентина', 'Чили', 'Колумбия', 'Перу', 'Уругвай', 'Парагвай', 'Европа', 'СНГ'],
    markets_sub2: 'Каналы',
    channels: ['Импортёры и дистрибьюторы', 'Аптечные сети и специализированная розница', 'Клиники и салоны', 'Маркетплейсы и электронная торговля'],
    markets_sub3: 'Категории',
    categories: ['K-beauty (OEM/ODM)', 'Медицина и эстетика', 'Судовое снабжение', 'Промышленное оборудование', 'Химия и материалы', 'Коммерческий транспорт'],
    markets_note: 'Легковые автомобили и запчасти к ним в наши задачи не входят. Мы не идём на рынок, где у вас уже есть официальный партнёр: скажите, какие территории закрыты, и они останутся закрытыми.',
    ...
    need: [
      { n: 'Письменное согласие', d: 'Одна строка в письме о том, что мы можем использовать ваши материалы для представления компании на платформе.' },
      { n: 'Каталог и изображения', d: 'Фотографии продукции и каталог. Названия и объёмы — так, как они стоят на упаковке: перед публикацией мы сверяем страницу с этикеткой.' },
      { n: 'Ваши сертификаты', d: 'Какие сертификаты и регистрации у вас есть и какие товары и рынки они покрывают. Копии нужны только на стадии сделки.' },
      { n: 'Закрытые территории', d: 'Страны, где у вас уже есть эксклюзивный или официальный партнёр, чтобы мы не привели вам покупателя, которого вы не сможете обслужить.' },
      { n: 'Цены — по желанию', d: 'Пришлите, если хотите, чтобы покупатель их видел. Если не хотите, на странице будет стоять «цена по запросу», и ничего не публикуется.' },
    ],

    cta_h: 'Готовы к публикации?',
    cta_d: 'Ответьте на это письмо или напишите нам. Мы соберём профиль из ваших материалов, пришлём ссылку на готовую страницу и внесём любую вашу правку в тот же день.',
  }),
};
